...
import {
  ...
  PlayCircle, ScrollText, Mic, MicOff, TrendingUp, Brain,
...
function getStoredProgress() {
  try { return JSON.parse(localStorage.getItem('arabic-dad-progress-v7') || '{}'); } catch { return {}; }
}
function saveStoredProgress(progress) {
  try { localStorage.setItem('arabic-dad-progress-v7', JSON.stringify(progress)); } catch {}
}
function getStoredMeta() {
  try {
    return JSON.parse(localStorage.getItem('arabic-dad-meta-v7') || '{"lessonsCompleted":0,"lastLessonNumber":0,"accomplishments":[],"lastGeneratedSession":"","lastCompletedSession":""}');
  } catch {
    return { lessonsCompleted: 0, lastLessonNumber: 0, accomplishments: [], lastGeneratedSession: '', lastCompletedSession: '' };
  }
}
function saveStoredMeta(meta) {
  try { localStorage.setItem('arabic-dad-meta-v7', JSON.stringify(meta)); } catch {}
}
function getStoredActiveLesson() {
  try { return JSON.parse(localStorage.getItem('arabic-dad-active-lesson-v7') || 'null'); } catch { return null; }
}
function saveStoredActiveLesson(activeLesson) {
  try { localStorage.setItem('arabic-dad-active-lesson-v7', JSON.stringify(activeLesson)); } catch {}
}
function clearStoredActiveLesson() {
  try { localStorage.removeItem('arabic-dad-active-lesson-v7'); } catch {}
}
...
function getCurrentSessionInfo() {
  ...
  return { session, sessionLabel, sessionId: `${dateKey}-${session}` };
}
...
function getBuiltInLesson(dayNumber, lessonKind = 'morning') {
  ...
  if (pool.length < 8) {
    ...
    pool = PHRASES.filter((p) =>
      categories.some((needle) => (p.track || '').toLowerCase().includes(needle) || (p.category || '').toLowerCase().includes(needle))
    );
  }
...
export default function App() {
  ...
  const [meta, setMeta] = useState({ lessonsCompleted: 0, lastLessonNumber: 0, accomplishments: [], lastGeneratedSession: '', lastCompletedSession: '' });
...
// This helps "prime" the voices so they are ready when the button is clicked
  ...
  useEffect(() => {
    ...
    if (active && active.sessionId === currentSessionId && Array.isArray(active.lesson) && active.lesson.length > 0 && active.stepIndex < active.lesson.length) {
  ...
  useEffect(() => {
    ...
    if (lesson.length > 0 && stepIndex < lesson.length) {
      saveStoredActiveLesson({ sessionId: currentSessionId, lesson, stepIndex, externalLessonInfo, lessonNumber, mode: 'lesson' });
    } else {
 ...
 const speak = (text) => {
  ...
  const registerLessonCompletion = () => {
    ...
    setMeta((prev) => {
      ...
      return { ...prev, lessonsCompleted: nextCompleted, lastLessonNumber: lessonNumber, lastCompletedSession: currentSessionId, accomplishments: nextAcc };
    });
  ...
  const goNext = () => {
    ...
    setStepIndex((s) => {
      const next = s + 1;
      if (next >= lesson.length) registerLessonCompletion();
      return next;
    });
  ...
  const checkTyping = () => {
    const ok = normalizeText(input) === normalizeText(current.answer || current.english) || normalizeText(input) === normalizeText(current.english);
  ...
  const resetAll = () => {
    localStorage.removeItem('arabic-dad-progress-v7');
    localStorage.removeItem('arabic-dad-meta-v7');
    localStorage.removeItem('arabic-dad-active-lesson-v7');
    setProgress({});
    setMeta({ lessonsCompleted: 0, lastLessonNumber: 0, accomplishments: [], lastGeneratedSession: '', lastCompletedSession: '' });
    setLessonNumber(1);
    setLesson([]);
    setStepIndex(0);
  ...
  const weakPractice = () => {
    ...
    setExternalLessonInfo({ count: weakPool.length || PHRASES.length, source: 'Weak-phrase practice' });
  ...
  const loadImportedLesson = () => {
    try {
      ...
      setMeta((prev) => ({ ...prev, lastGeneratedSession: currentSessionId }));
    } catch (err) {
  ...
  const generateLesson = useCallback(async () => {
    try {
      setGenerating(true); setGenerateError(''); setImportError('');
  ...
  useEffect(() => {
    ...
    if (!alreadyGeneratedThisSession && !alreadyCompletedThisSession) generateLesson();
  ...
  "source_phrases": [
    {
      ...
      "transliteration": "Tahmilu al-mudunu al-qadima dhakiratan tawila",
  ...
  const renderNavButtons = (primaryLabel = 'Next') => (
    <div className="button-grid-2">
      <Button secondary onClick={goBack} disabled={stepIndex === 0}>Back</Button>
      <Button onClick={goNext}>{primaryLabel}</Button>
    </div>
  );
    ...
    <div className="app-shell">
      <div className="app-container">
        <div className="card">
          ...
          <div className="progress-card"><div className="row-between"><span>{currentSessionLabel} lesson</span><span>{lessonProgress}%</span></div><div className="progress-bar thin"><div style={{ width: `${lessonProgress}%` }} /></div></div>
        </div>
        ...
        <div className="card">
          ...
          <div className="button-grid-2 top-gap">
            ...
            <Button secondary onClick={() => {
              ...
              setLesson(built);
              setExternalLessonInfo({ count: built.length, source: 'Built-in lesson' });
              setStepIndex(0); setInput(''); setFeedback(null); setShowAnswer(false); setGenerateError(''); setImportError(''); setMode('lesson');
              setMeta((prev) => ({ ...prev, lastGeneratedSession: currentSessionId }));
            }}>Use built-in lesson</Button>
          </div>
        ...
        {mode === 'lesson' && <div className="card">
          {noLessonLoaded ? <div className="lesson-center">
            <div className="title-big">{meta.lastCompletedSession === currentSessionId ? `${currentSessionLabel} session complete` : 'Lesson ready soon'}</div>
            <div className="muted">{meta.lastCompletedSession === currentSessionId ? 'You have already finished this session. A new lesson will appear in the next session.' : generating ? 'Generating your lesson...' : 'Tap Generate lesson to start.'}</div>
            <div className="stack top-gap"><Button onClick={generateLesson} disabled={generating}>{generating ? 'Generating...' : 'Generate lesson'}</Button></div>
          </div> : lessonComplete ? <div className="lesson-center">
          ...
          <div className="stack top-gap">
            {(track === 'All Tracks' ? PHRASES : PHRASES.filter((p) => p.track === track)).map((item) => (
              <div key={item.id} className="phrase-card">
                ...
                <div className="arabic-mid top-gap" dir="rtl">{item.arabic}</div><div className="translit small">{item.transliteration}</div><div className="english-mid">{item.english}</div>
              </div>
            ))}
          </div>
        </div>}
        ...
        <div className="card">
          <details>
            <summary style={{ cursor: 'pointer', fontWeight: '600' }}>Manual lesson import</summary>
            <div className="top-gap">
              <p className="muted">Paste JSON here only if you want to override the generated lesson.</p>
