...
import {
  ...
  PlayCircle, ScrollText, Mic, MicOff, TrendingUp, Brain, Trophy
} from 'lucide-react';
import { FORTNIGHTLY_LESSONS } from './fortnightlyLessons';
...
function getStoredProgress() {
  try {
    return JSON.parse(localStorage.getItem('arabic-fortnight-progress-v1') || '{}');
  } catch {
    return {};
  }
}
...
function saveStoredProgress(progress) {
  try {
    localStorage.setItem('arabic-fortnight-progress-v1', JSON.stringify(progress));
  } catch {}
}
...
function getStoredMeta() {
  try {
    return JSON.parse(
      localStorage.getItem('arabic-fortnight-meta-v1') ||
        '{"lessonsCompleted":0,"lastLessonNumber":0,"accomplishments":[],"lastCompletedSessionKey":""}'
    );
  } catch {
    return {
      lessonsCompleted: 0,
...
function saveStoredMeta(meta) {
  try {
    localStorage.setItem('arabic-fortnight-meta-v1', JSON.stringify(meta));
  } catch {}
}

function getStoredActiveLesson() {
  try {
    return JSON.parse(localStorage.getItem('arabic-fortnight-active-lesson-v1') || 'null');
  } catch {
    return null;
  }
}
...
function saveStoredActiveLesson(activeLesson) {
  try {
    localStorage.setItem('arabic-fortnight-active-lesson-v1', JSON.stringify(activeLesson));
  } catch {}
}

function clearStoredActiveLesson() {
  try {
    localStorage.removeItem('arabic-fortnight-active-lesson-v1');
  } catch {}
}
...
function getCurrentSessionInfo() {
  ...
  const fortnightDay = ((dateOfMonth - 1) % 15) + 1;
  ...
  return {
    session,
    sessionLabel,
    sessionKey: `${dateKey}-${session}`,
    fortnightDay
  };
}

function getFortnightLesson(day, session) {
  return (
    FORTNIGHTLY_LESSONS.find(
      (lesson) => lesson.day === day && lesson.session === session
    ) || null
  );
}

function flattenAllFortnightPhrases() {
  const all = [];
  for (const lesson of FORTNIGHTLY_LESSONS) {
    for (const phrase of lesson.source_phrases || []) {
      all.push({
        ...phrase,
        answer: (phrase.answer || phrase.english || '').toLowerCase()
      });
    }
  }
  return all;
...
function generateTasksFromPhrases(base, lessonKind = 'morning') {
  ...
  for (let i = 0; i < selected.length; i++) {
    const item = {
      ...selected[i],
      answer: (selected[i].answer || selected[i].english || '').toLowerCase()
    };
...
export default function App() {
  ...
  const currentSessionKey = sessionInfo.sessionKey;
  const fortnightDay = sessionInfo.fortnightDay;
  ...
  const allPhrases = useMemo(() => flattenAllFortnightPhrases(), []);
  const currentFortnightLesson = useMemo(
    () => getFortnightLesson(fortnightDay, currentSession),
    [fortnightDay, currentSession]
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (currentFortnightLesson?.source_phrases?.length) {
        const builtLesson = generateTasksFromPhrases(
          currentFortnightLesson.source_phrases,
          currentSession
        );
        setLesson(builtLesson);
        setStepIndex(0);
      }
    }

    setInitLoaded(true);
  }, [currentSessionKey, currentSession, currentFortnightLesson]);
  ...
  }, [initLoaded, currentSessionKey, lesson, stepIndex, lessonNumber]);

  const currentStep = lesson[stepIndex] || null;
  const current = currentStep?.item || allPhrases[0] || null;
  ...
  const overallProgress = allPhrases.length
    ? Math.min(100, Math.round((masteredCount / allPhrases.length) * 100))
    : 0;
  ...
  const categoryStats = useMemo(() => {
    const cats = [...new Set(allPhrases.map((p) => p.category))];
    return cats.slice(0, 12).map((cat) => {
      const items = allPhrases.filter((p) => p.category === cat);
  ...
  const registerLessonCompletion = () => {
    ...
    setMeta((prev) => {
      ...
      return {
        ...
        lastCompletedSessionKey: currentSessionKey,
  ...
  const checkTyping = () => {
    if (!current) return;
    const ok =
      normalizeText(input) === normalizeText(current.answer || current.english) ||
  ...
  const startVoicePractice = () => {
    if (!voiceSupported || typeof window === 'undefined' || !current) return;
  ...
  const weakPractice = () => {
    const weakPool = allPhrases.filter((p) => {
      const s = getItemStats(progress, p.id);
      return s.wrong >= s.correct || !s.mastered;
    });

    const source = weakPool.length ? weakPool : allPhrases;
    setLesson(generateTasksFromPhrases(shuffle(source).slice(0, 8), 'evening'));
    setStepIndex(0);
    setInput('');
    setFeedback(null);
    setShowAnswer(false);
      ...
      <div className="app-container">
        <div className="card">
          ...
          <div className="stats-grid stats-grid-4">
            ...
            <div className="mini-card"><span>Library size</span><strong>{allPhrases.length}</strong></div>
          ...
          <div className="progress-card">
            <div className="row-between">
              <span>Fortnight Day {fortnightDay} · {currentSessionLabel}</span>
        ...
        <div className="card">
          ...
          <div className="button-grid-2 top-gap">
            <button className="toggle-card">
              <Brain size={16} /> {currentSessionLabel} lesson
            </button>
        ...
        {mode === 'lesson' && (
          <div className="card">
            {noLessonLoaded ? (
              <div className="lesson-center">
                <div className="title-big">No lesson loaded</div>
                <div className="muted">
                  Add the lesson for Fortnight Day {fortnightDay} ({currentSession}) in fortnightlyLessons.js
                </div>
              </div>
            ) : lessonComplete ? (
              <div className="lesson-center">
                <div className="title-big">Lesson {lessonNumber - 1} complete 🎉</div>
                <div className="muted">A clean stopping point for this session.</div>
              ...
              <>
                <div className="row-between">
                  <span className="pill soft">Fortnight Day {fortnightDay} · Step {stepIndex + 1} / {lesson.length}</span>
                  <span className="muted">{currentFortnightLesson?.title || current.track}</span>
                </div>
        ...
        {mode === 'listen' && (
          <div className="card">
            ...
            <div className="stack top-gap">
              {allPhrases.slice(0, 14).map((item) => (
        ...
        {mode === 'phrasebook' && (
          <div className="card">
            <h2>Phrasebook</h2>
            <p className="muted">Browse the current fortnight’s phrases.</p>
            <div className="stack top-gap">
              {allPhrases.map((item) => (
        ...
        {mode === 'review' && (
          <div className="card">
            ...
            <p className="muted">Track completed lessons and revisit weaker phrases.</p>
            ...
            <div className="stats-grid stats-grid-3 top-gap">
              ...
              <div className="mini-card"><span>Phrase bank</span><strong>{allPhrases.length}</strong></div>
            </div>
              ...
              {categoryStats.map((row) => (
                <div key={row.cat} className="phrase-card">
                  ...
                  <div className="progress-bar thin top-gap">
                    <div style={{ width: `${row.total ? (row.mastered / row.total) * 100 : 0}%` }} />
                  </div>
                </div>
              ))}
            </div>
...
path = Path('/mnt/data/App_fortnight_mobile.jsx')
path.write_text(code, encoding='utf-8')
print(path)
print("saved")
.`|`
analysis to=python_user_visible.exec codeimport React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import {
  Volume2, CheckCircle2, XCircle, RotateCcw, BookOpen, Ear, Shuffle, Star,
